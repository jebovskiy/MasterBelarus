    ...
    <div className="min-h-screen bg-app-bg">
      <nav className="sticky top-0 z-40 bg-app-bg/80 backdrop-blur-md border-b border-app-border">
        <div className="max-w-[430px] mx-auto flex items-center justify-around px-4 h-14">
          <button
            onClick={() => { setTab('client'); impact('light'); }}
            className={`flex flex-col items-center gap-0.5 text-xs font-semibold transition ${tab !== 'master' ? 'text-primary' : 'text-text-muted'}`}
          >
            <span className="text-lg">🏠</span>
            Клиенту
          </button>
          <button
            onClick={() => { setTab('master'); impact('light'); }}
            className={`flex flex-col items-center gap-0.5 text-xs font-semibold transition ${tab === 'master' ? 'text-primary' : 'text-text-muted'}`}
          >
            <span className="text-lg">👷</span>
            Мастеру
      ...
      {tab !== 'master' ? (
        <ClientHome onOpenCreateOrder={() => setOrderOpen(true)} onOpenOrder={setSelectedOrderId} />
      ) : (
        <MasterHome onOpenOrder={setSelectedOrderId} />
      )}
