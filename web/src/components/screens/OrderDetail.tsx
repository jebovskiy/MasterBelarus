...
function emoji(cat: string) {
  return cat === 'plumber' ? '🔧' : cat === 'electrician' ? '⚡' : cat === 'mover' ? '📦' : cat === 'handyman' ? '🛠' : cat === 'tutor' ? '📚' : cat === 'cleaning' ? '🧹' : '📋';
}
...
function statusBadge(status: string) {
  ...
  const label = { open: 'Открыт', in_progress: 'В работе', completed: 'Завершён', cancelled: 'Отменён' }[status] ?? status;
  return <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${map[status] ?? 'bg-gray-100'}`}>{label}</span>;
}
...
export default function OrderDetail({ orderId, onBack }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setOrder(orderRes as OrderRow);
      setBids((bidsRes as unknown as Bid[]) ?? []);
    } catch (e) {
  ...
  const submitReview = async () => {
    ...
    impact('medium');
    const res = await apiPost(`/orders/${orderId}/review`, {
      rating: reviewRating,
    ...
    <div className="fixed inset-0 z-50 bg-app-bg overflow-auto">
      <header className="sticky top-0 z-10 bg-app-bg/80 backdrop-blur-md border-b border-app-border">
        <div className="max-w-[430px] mx-auto flex items-center justify-between px-4 h-14">
          <button onClick={onBack} className="px-3 h-8 rounded-btn bg-app-surface-alt text-sm font-semibold">
            Назад
          </button>
      ...
      <div className="max-w-[430px] mx-auto px-4 py-4 space-y-4">
        {error && (
          <div className="bg-red-50 text-error p-3 rounded-bento text-sm">
            {error}
            <button onClick={load} className="ml-2 underline font-semibold">Повторить</button>
          </div>
        )}
        ...
        {loading && (
          <div className="space-y-3">
            {[0, 1, 2].map((i) => (
              <div key={i} className="h-28 rounded-bento bg-white shadow-card animate-pulse" />
            ))}
          </div>
        )}
          ...
          <>
                ...
                <div className="space-y-3">
                  {bids.map((bid, idx) => (
                    <motion.div
                      ...
                      className="bg-white p-4 rounded-bento shadow-card"
                    >
                      <div className="flex items-center justify-between mb-2">
                        <div>
                          <p className="text-sm font-semibold text-text-main">
                            Мастер #{bid.master_id.slice(0, 6)}
                          </p>
                          <p className="text-xs text-text-muted">{timeAgo(bid.created_at)}</p>
                        </div>
                      ...
                      <button
                        ...
                        className="w-full h-10 rounded-btn bg-primary hover:bg-primary-hover text-white text-sm font-semibold disabled:opacity-50 active:scale-[0.98] transition"
                      >
            ...
            {order.status === 'in_progress' && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-white p-5 rounded-bento shadow-card">
                ...
                <div className="flex gap-2 mb-3">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <button
                      ...
                      className={`text-2xl transition ${star <= reviewRating ? 'scale-110' : 'opacity-30'}`}
                    >
                      ★
                    </button>
                  ))}
                ...
                <textarea
                  ...
                  className="w-full rounded-btn border border-app-border bg-white p-3 text-sm mb-3"
                />
                <button
                  onClick={submitReview}
                  disabled={reviewSubmitting}
                  className="w-full h-11 rounded-btn bg-primary hover:bg-primary-hover text-white font-semibold disabled:opacity-50"
                >
            ...
            {order.status === 'completed' && (
              <div className="text-center py-6">
                ...
                <p className="text-sm font-semibold text-text-main mt-2">Заказ завершён</p>
              </div>
            )}
          </>
        )}
